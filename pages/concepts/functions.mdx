...
export const processOrder = inngest.createFunction(
  ...
  { 
    id: "process-order",
    concurrency: { limit: 10 }, // See: https://www.inngest.com/docs/guides/concurrency
    retries: 3 // See: https://www.inngest.com/docs/features/inngest-functions/error-retries/retries
  },
  // 2. Trigger
  { event: "order/created" }, // See: https://www.inngest.com/docs/features/events-triggers
  // 3. Handler
  async ({ event, step }) => {
...
export const generateDailyReport = inngest.createFunction(
  { id: "daily-report" },
  { cron: "0 9 * * *" }, // See: https://crontab.guru/
  async ({ step }) => {
...
```typescript
{
  ...
  concurrency: { limit: 5, key: "event.data.userId" }, // Guide: /concepts/flow-control#concurrency-control
  
  // Control execution frequency  
  rateLimit: { limit: 100, period: "1h" }, // Guide: /concepts/flow-control#rate-limiting
  
  // Process events in batches
  batchEvents: { maxSize: 50, timeout: "10s" }, // Guide: /concepts/flow-control#event-batching
  
  // Handle failures
  retries: 3, // Docs: https://www.inngest.com/docs/features/inngest-functions/error-retries/retries
  onFailure: async ({ error }) => alerting.send(error.message) // Docs: https://www.inngest.com/docs/reference/functions/handling-failures
}
```
...
You can invoke other functions synchronously to wait for their completion using `step.invoke()`, or trigger them asynchronously using `step.sendEvent()` ([see Event Chains](/concepts/events#event-chains)).
...
1. **Use descriptive step names** - They appear in logs and debugging. [More on Naming Steps](/concepts/steps#step-best-practices).
2. **Keep steps atomic** - Each step should do one thing well. [More on Atomicity](/concepts/steps#2-keep-steps-atomic).
3. **Handle errors gracefully** - Use try/catch for business logic errors. [Error Handling Patterns](/quick-start/patterns#error-handling-in-steps).
4. **Leverage type safety** - Define event types for better development experience. [TypeScript with Inngest](https://www.inngest.com/docs/typescript).
5. **Monitor function performance** - Use the [Inngest dashboard](https://www.inngest.com/docs/platform/monitor) to track metrics.
