...
export const processOrderWithSteps = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    ); // See: step.run reference (https://www.inngest.com/docs/reference/functions/step-run)
...
Each step executes in isolation and communicates with Inngest's execution engine. When a step completes successfully, its result is stored permanently and will never re-execute, even if the function is retried. If a step fails, only that specific step retries while all previous successful steps return their [memoized](/concepts/steps#memoization) results. [Dive deeper into the step lifecycle](https://www.inngest.com/docs/features/inngest-functions/steps-workflows/step-execution-lifecycle).
...
Pause a workflow for a specific duration, from seconds to days using [`step.sleep()`](https://www.inngest.com/docs/reference/functions/step-sleep). This is perfect for creating drip email campaigns, reminder systems, or any process that needs to wait for a period before continuing. The state of the workflow is preserved during the sleep. [See sleep use cases](https://www.inngest.com/docs/guides/scheduled-functions#sleeps-for-dynamic-schedules).
...
Pause a workflow until a specific external event occurs using [`step.waitForEvent()`](https://www.inngest.com/docs/reference/functions/step-wait-for-event). This is ideal for approval steps, waiting for user actions, or integrating with asynchronous third-party systems. The function remains dormant until the target event is received, then resumes with the event's data. [See waitForEvent use cases](https://www.inngest.com/docs/guides/wait-for-event) and [event matching options](https://www.inngest.com/docs/reference/functions/step-wait-for-event#arguments).
...
Execute multiple asynchronous operations at the same time using [`step.parallel()`](https://www.inngest.com/docs/reference/functions/step-parallel) and wait for all of them to complete before proceeding. This is useful for fetching data from multiple sources, processing items in a collection independently, or any task where operations can run in parallel to speed up execution. Each parallel operation is still a durable step. [See parallel execution guide](https://www.inngest.com/docs/guides/parallel-steps).
