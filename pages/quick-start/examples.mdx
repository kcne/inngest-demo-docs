...
type UserSignupEvent = {
  name: "user/signup"; // See: Event Naming Best Practices (/quick-start/patterns#use-specific-event-names)
  data: {
    userId: string;
    email: string;
    name: string;
...
import { NonRetriableError } from "inngest"; // See: https://www.inngest.com/docs/reference/typescript/inngest#nonretriableerror
...
export const userOnboardingSequence = inngest.createFunction(
  {
    ...
    concurrency: { 
      ...
      limit: 1 
    }, // See: /concepts/flow-control#concurrency-keys-multi-tenant-concurrency
    // Configure retries for transient errors
    retries: 3 // See: https://www.inngest.com/docs/features/inngest-functions/error-retries/retries
  },
  ...
  async ({ event, step, logger }) => {
    ...
    const activationEvent = await step.waitForEvent("wait-for-feature-activation", {
      ...
      if: `event.data.userId == "${userId}"`, // See: https://www.inngest.com/docs/reference/functions/step-wait-for-event#if-expression
    });
  ...
  async ({ event, step, logger }) => {
    ...
    try {
      ...
      await step.sendEvent("send-payment-failed-notification", {
        name: "user/payment_failed", // See: /quick-start/patterns#use-specific-event-names
...
**Why Inngest Excels Here:**
Inngest simplifies complex orchestrations. [`step.invoke()`](/concepts/steps#stepinvoke---composable-workflows) allows breaking down the monolith into manageable, independently testable, and durable functions. Error handling within steps ensures issues like payment declines are handled gracefully without derailing the entire workflow.
...
**Why Inngest Excels Here:**
Complex dunning logic that spans days or weeks is notoriously hard to build with traditional job queues. Inngest's `step.sleep()` and durable step retries simplify this immensely, allowing you to write straightforward loops and conditional logic that reliably manage the entire subscription lifecycle.

**Learn More:**
- [Official Guide: Building Subscription Billing](https://www.inngest.com/docs/guides/subscription-billing-template) (if a specific guide exists, otherwise use more general links)
...
export const contentModerationPipeline = inngest.createFunction(
  {
    ...
    concurrency: { limit: 20 } // See: /concepts/flow-control#concurrency-control
  },
...
**Why Inngest Excels Here:**
Human-in-the-loop workflows are often complex. [`step.waitForEvent()`](/concepts/steps#stepwaitforevent---wait-for-external-input) simplifies this by allowing a workflow to pause indefinitely (or with a timeout) until a specific action occurs, all while maintaining state and context without external databases for tracking.

**Learn More:**
- [Official Guide: Human in the Loop Workflows](https://www.inngest.com/docs/guides/human-in-the-loop) (if available)
...
- [Guide: Large Scale Data Processing with Inngest](https://www.inngest.com/docs/guides/batch-processing) (if a specific guide exists)
